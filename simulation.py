...
xllcorner, yllcorner = 0, 0  # Lower-left corner coordinates

# Define the domain
x = np.linspace(xllcorner, xllcorner + Nx * dx, Nx + 2)  # Include ghost cells
y = np.linspace(yllcorner, yllcorner + Ny * dy, Ny + 2)  # Include ghost cells
X, Y = np.meshgrid(x, y)

# Define the slope: z = ax + by + c
a, b, c = 0.1, 0.2, 0  # Slope coefficients
Z = a * X + b * Y + c  # Compute the slope

# Plot the slope as a 3D surface
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')

# Surface plot
surf = ax.plot_surface(X, Y, Z, cmap='viridis', edgecolor='k', alpha=0.8)
fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10, label='Slope (Z)')

# Labels and title
ax.set_title('3D Slope Visualization')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')

# Save the plot as a PNG image
output_file = "3d_slope_plot.png"
